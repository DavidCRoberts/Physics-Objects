...
#updated by Kalyan Khatiwada for CS 152 B, Project 8
...
def buildObstacles(win):
    """Support funciton to set up obstacles and walls for the pinball simulation"""
    #create four block borders to encompass the playing field
    ...
    #create obstacles of different shapes within the borders
...
def main():
    """Function that uses multiple objets and methods to create a pinball simulation"""
    ...
    tetra = pho.Tetrahedral_Molecule(win)
    tetra.setRadius(0.8)
    tetra.setColor([250,150,150])
    tetra.setVelocity(random.randint(-10, 10), 10)
    tetra.setAcceleration(0, 0)
    tetra.setPosition(17,6)
    tetra.draw()
    tri = pho.Triangle(win)
    tri.setRadius(0.8)
    tri.setAcceleration(10,1)
    tri.setColor([250,150,150])
    tri.setHeight(3)
    tri.setMass(1)
    tri.setWidth(3)
    tri.setVelocity(random.randint(-10, 10), 10)
    tri.setPosition(17,6)
    tri.draw()
    ...
    while True:
        ...
        if keypressed == "q":
            break
        if keypressed =="n":
            newobs = pho.Block(win, color = (0, 100, 100))
            newobs.setPosition(20, 8)
            newobs.draw()
            shapes.append(newobs)
            new_obs_list = [newobs] 
        if keypressed == "w": # if the user pressed the up arrow key, move the most recently created obstacle created up
        ...
        if keypressed == "s": # if the user pressed the down arrow key, move the most recently created obstacle created down
            for obs in new_obs_list:
                obs_pos = obs.getPosition()
                x_old = obs_pos[0]
                y_old = obs_pos[1]
        ...
        if keypressed == "a": # if the user pressed the left arrow key, move the most recently created obstacle created left
            for obs in new_obs_list:
                obs_pos = obs.getPosition()
                x_old = obs_pos[0]
                y_old = obs_pos[1]
        ...
        if keypressed == "d": # if the user pressed the right arrow key, move the most recently created obstacle created right
            for obs in new_obs_list:
                obs_pos = obs.getPosition()
                x_old = obs_pos[0]
                y_old = obs_pos[1]
    ...
    #reset ball if it goes out of bounds
        ...
        if pos[0] < 10 or pos[0] > 30 or pos[1] < 7:
            ...
            tetra.setVelocity(random.randint(-10, 10), 10)
            tetra.setAcceleration(0, 0)
        pos2 = tri.getPosition()
        if pos2[0] < 10 or pos2[0] > 30 or pos2[1] < 7:
            tri.setPosition(20, 8)
            tri.setVelocity(random.randint(-10, 10), 10)
            tri.setAcceleration(0, 0)
        ...
        if collided == False:
            # call the update method of the balls with dt as the time step
            tri.update(dt)
            tetra.update(dt)
